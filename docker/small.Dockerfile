...
RUN cd /tmp \
 ...
 && tlmgr paper a4 \
 && tlmgr install collection-latex collection-langjapanese xkeyval etoolbox ms filehook unicode-math lualatex-math xcolor fancyvrb booktabs mdwtools ulem
# if there is not a necessary TeX style file in this image, find a package composing it by the command below:
